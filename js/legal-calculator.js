...
«___» ________ 202_ г. мною в вашей организации был приобретен товар: _________________________________________ стоимостью ${data.productPrice.toLocaleString('ru-RU')} руб.
...
2. Законная неустойка 1% в день (ст. 23 ЗоЗПП на день претензии): ${data.penaltyAmount.toLocaleString('ru-RU')} руб.
...
Приложения:
1. Копия документа, подтверждающего приобретение и оплату товара (кассовый/товарный чек, выписка по счету);
...
Между мною и вашей организацией был заключен договор № _______ от «___» ________ 202_ г. на выполнение работ / оказание услуг общей стоимостью ${data.servicePrice.toLocaleString('ru-RU')} руб.
В нарушение условий договора и требований ст. 27, 28 Закона РФ «О защите прав потребителей» установленные договором сроки были нарушены. Период просрочки составил ${data.delayDays} дней.
...
У должника имеется неисполненное денежное обязательство в размере ${data.debtAmount.toLocaleString('ru-RU')} руб.
...
При заключении кредитного договора мною был оплачен договор страхования / пакет дополнительных финансовых услуг стоимостью ${data.baseRefund.toLocaleString('ru-RU')} руб.
${isCool 
    ? `С момента заключения договора прошло ${data.daysPassed} дней, что укладывается в гарантированный законом 30-дневный «период охлаждения» (ч. 2.4 ст. 7 Федерального закона № 353-ФЗ «О потребительском кредите (займе)»). В период охлаждения потребитель имеет безусловное право на 100% возврат уплаченной суммы.`
...
Я являюсь потребителем коммунальной услуги по отоплению в жилом помещении по адресу: ____________________________________________________.
...
В жилом помещении по адресу: ____________________________________________________ имело место непредоставление коммунальной услуги водоснабжения общей продолжительностью ${data.outageHours} часов, что превышает установленный норматив (не более 8 часов суммарно в месяц, не более 4 часов единовременно) на ${data.excessHours} ч.
...
В жилом помещении по адресу: ____________________________________________________ проведена поверка индивидуального прибора учета, подтвердившая исправность прибора. Сведения о поверке внесены во ФГИС «АРШИН».
...
Сумма излишне начисленной платы, подлежащая зачету: ${data.totalRefund.toLocaleString('ru-RU')} руб.
...
Я и члены моей семьи временно отсутствовали в жилом помещении по адресу: ____________________________________________________ в течение ${data.absentDays} полных календарных дней подряд.
На основании раздела VIII Правил (ПП РФ № 354) прошу произвести перерасчет платы за коммунальную услугу обращения с ТКО / коммунальные услуги.

I. СУММА ПЕРЕРАСЧЕТА:
Сумма перерасчета к зачету в счет будущих периодов: ${data.refundAmount.toLocaleString('ru-RU')} руб.
...
Я состою (состоял) в трудовых отношениях с работодателем.
...
Я являюсь собственником жилого помещения по адресу: ____________________________________________________.
