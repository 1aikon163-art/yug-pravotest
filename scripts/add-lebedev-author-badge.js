...
htmlFiles.forEach(filePath => {
  ...
  // Replace standard copyright line with Lebedev badge
  if (html.includes('© 2026 АНО «ЦПЗ ЮГ-ПРАВО». Все права защищены.')) {
    html = html.replace(
      /<div class="text-xs text-\[#5f5e5e\]">© 2026 АНО «ЦПЗ ЮГ-ПРАВО»\. Все права защищены\.<\/div>/gi,
      lebedevFooterBadge
    );
    html = html.replace(
      /© 2026 АНО «ЦПЗ ЮГ-ПРАВО»\. Все права защищены\./g,
      `© 2026 АНО «ЦПЗ ЮГ-ПРАВО». Все права защищены. | Архитектура, код и дизайн: <a href="https://t.me/aikon163" target="_blank" rel="noopener noreferrer" class="font-bold underline hover:text-[#C5A059]">П. В. Шарыпаев</a>`
    );
    changed = true;
  }
  ...
  if (changed) {
    ...
    console.log(`Added Lebedev-style badge to: ${path.relative(rootDir, filePath)}`);
  }
});

// Upload root HTML files to VPS
const conn = new Client();
conn.on('ready', () => {
  console.log('SSH connected. Uploading Lebedev-style author badges to VPS...');
  conn.sftp((err, sftp) => {
    if (err) throw err;
        ...
        sftp.fastPut(p, `/var/www/yug-pravo/${file}`, () => {
          ...
          if (count === rootHtmlFiles.length) {
            conn.exec('systemctl reload nginx', () => {
              console.log('LEBEDEV_AUTHOR_BADGE_DEPLOYED_SUCCESSFULLY');
              conn.end();
            });
          }
        });
