...
function updateFaviconVersion(dir, prefix = '') {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      ...
      html = html.replace(/href=["'][^"']*favicon\.png[^"']*["']/gi, `href="${prefix}favicon.png?v=4"`);
...
conn.on('ready', () => {
  console.log('SSH connected. Uploading HTMLs with v=4 real 3D render...');
  conn.sftp((err, sftp) => {
    if (err) throw err;
      ...
      sftp.fastPut(path.join(rootDir, hf), `/var/www/yug-pravo/${hf}`, () => {
        ...
        if (count === htmlFiles.length) {
          conn.exec('systemctl reload nginx', () => {
            console.log('REAL_3D_FAVICON_V4_DEPLOYED_EVERYWHERE');
            conn.end();
          });
        }
      });
