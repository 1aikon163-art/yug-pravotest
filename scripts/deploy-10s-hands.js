...
// 1. Configure index.html with clean video tag (like initiatives.html)
let indexPath = path.join(rootDir, 'index.html');
let indexHtml = fs.readFileSync(indexPath, 'utf8');

const cleanIndexVideo = `<video id="hands-video" 
                    src="hands.mp4" 
...
indexHtml = indexHtml.replace(/<video id="hands-video"[\s\S]*?<\/video>/i, cleanIndexVideo);
fs.writeFileSync(indexPath, indexHtml, 'utf8');

// 2. Configure about.html with clean video tag (like disclosure.html)
let aboutPath = path.join(rootDir, 'about.html');
let aboutHtml = fs.readFileSync(aboutPath, 'utf8');

const cleanAboutVideo = `<video id="about-hero-video" 
                    src="12.mp4" 
...
aboutHtml = aboutHtml.replace(/<video id="about-hero-video"[\s\S]*?<\/video>/i, cleanAboutVideo);
fs.writeFileSync(aboutPath, aboutHtml, 'utf8');

// 3. Update js/main.js with accurate 10-second duration and open hands start
let mainJsPath = path.join(rootDir, 'js/main.js');

const perfect10sMainJs = `/**
  ...
  function initMode() {
    ...
    if (!isDesktop) {
      ...
      if (isPingPong) {
        ...
        // ⚡ РЕЖИМ СКРАББИНГА ПО СКРОЛЛУ
        const isHands = activeVideo.id === 'hands-video' || activeVideo.dataset.scrubDirection === 'reverse';
        
        // hands.mp4 точная длительность 10.0с, остальные ~4.8с
        ...
        // Устанавливаем начальный фрейм (раскрытые руки)
        const setOpenHandsStart = () => {
          maxDuration = getVideoDuration();
          if (isHands) {
            targetTime = maxDuration;
            smoothedTime = maxDuration;
            try {
        ...
        if (activeVideo.readyState >= 1) {
          ...
          activeVideo.addEventListener('loadedmetadata', setOpenHandsStart, { once: true });
          activeVideo.addEventListener('canplay', setOpenHandsStart, { once: true });
        }
        ...
        const onScroll = () => {
          ...
          if (maxScroll > 0) {
            ...
            // При скролле вниз (progress 0 -> 1) руки смыкаются (10.0 -> 0)
...
fs.writeFileSync(mainJsPath, perfect10sMainJs, 'utf8');

// 4. Upload to VPS
const conn = new Client();
conn.on('ready', () => {
  console.log('SSH connected. Uploading exact 10s hands setup to VPS...');
  conn.sftp((err, sftp) => {
    if (err) throw err;
      ...
      sftp.fastPut(aboutPath, '/var/www/yug-pravo/about.html', () => {
        sftp.fastPut(mainJsPath, '/var/www/yug-pravo/js/main.js', () => {
          conn.exec('systemctl reload nginx', () => {
            console.log('HANDS_10S_AND_ABOUT_DEPLOYED_SUCCESSFULLY');
            conn.end();
          });
        });
      });
