...
function updateFaviconVersion(dir, prefix = '') {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      ...
      html = html.replace(/href=["'][^"']*favicon\.png[^"']*["']/gi, `href="${prefix}favicon.png?v=7"`);
...
conn.on('ready', () => {
  console.log('SSH connected. Uploading HTMLs with v=7 version bump...');
  conn.sftp((err, sftp) => {
    if (err) throw err;
      ...
      sftp.fastPut(path.join(rootDir, hf), `/var/www/yug-pravo/${hf}`, () => {
        ...
        if (count === htmlFiles.length) {
          conn.exec('systemctl reload nginx', () => {
            console.log('V7_GLASS_DEPLOYMENT_COMPLETE');
            conn.end();
          });
        }
      });
