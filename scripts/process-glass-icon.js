...
async function processGlassIcon() {
  console.log('Processing Duo-Tone Glass Icon for Favicon...');
  const img = sharp(inputPath);
  const { data, info } = await img.ensureAlpha().raw().toBuffer({ resolveWithObject: true });
  const pixelCount = info.width * info.height;
  const outputBuffer = Buffer.alloc(pixelCount * 4);

  // Background removal outside the frosted glass squircle
  for (let i = 0; i < pixelCount; i++) {
    const r = data[i * 4];
    const g = data[i * 4 + 1];
    const b = data[i * 4 + 2];
    
    // Whiteness threshold for outer background
  ...
  const transparentPng = await sharp(outputBuffer, {
    raw: { width: info.width, height: info.height, channels: 4 }
  }).trim().png().toBuffer();

  // Save 256x256 favicon
  ...
  console.log('Glass Duo-Tone Favicon created successfully!');

  // Upload to VPS
  const conn = new Client();
  conn.on('ready', () => {
    console.log('SSH connected. Uploading glass duo-tone favicon...');
    conn.sftp((err, sftp) => {
      if (err) throw err;
      ...
      sftp.fastPut(path.join(rootDir, 'favicon.png'), '/var/www/yug-pravo/favicon.png', () => {
        sftp.fastPut(path.join(rootDir, 'images/favicon.png'), '/var/www/yug-pravo/images/favicon.png', () => {
          conn.exec('systemctl reload nginx', () => {
            console.log('GLASS_DUOTONE_FAVICON_DEPLOYED');
            conn.end();
          });
        });
      });
...
processGlassIcon().catch(console.error);
